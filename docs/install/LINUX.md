...
**Russian:** [docs/ru/install/LINUX.md](../ru/install/LINUX.md)

Native installation without Docker: PHP-FPM or `artisan serve`, PostgreSQL, Redis, Nginx/Apache.  
...
2. Creates `.env` from `.env.production.example` or `.env.example`
3. `composer install`
4. `php artisan key:generate`
5. `php artisan migrate --force`
6. `php artisan app:bootstrap --force`
7. `npm ci && npm run build`
8. Health gate at `http://localhost/health` (if the web server is already running)
