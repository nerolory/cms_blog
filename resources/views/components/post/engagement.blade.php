...
@unless ($fragmentOnly)
    <div data-post-engagement-app data-engagement-url="{{ route('posts.engagement.show', $post) }}"
        data-stream-url="{{ route('posts.engagement.stream', $post) }}"
        data-engagement-version="{{ $engagementVersion }}">
@endunless
...
<div class="post-engagement mb-4">
    ...
    @auth
        <form method="POST" action="{{ route('posts.reactions.store', $post) }}"
            class="d-flex flex-wrap gap-2 mb-4" data-engagement-reaction-form>
            @csrf
    ...
    <section class="mt-4" data-comments-section data-post-slug="{{ $post->slug }}"
        ...
        <div data-comment-roots>
            ...
            @else
                <x-post.comments-roots-chunk :post="$post" :roots="$comments->rootComments"
                    :reply-counts="$comments->replyCounts" :reaction-summaries="$comments->reactionSummaries" />
            @endif
